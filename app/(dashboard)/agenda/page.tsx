...
export default function AgendaPage() {
  ...
  const { events, isLoading, refetch } = useCalendarEvents({ from, to })
  ...
  return (
    <div className="flex flex-col h-full overflow-y-auto scroll-container p-2 sm:p-4">
      {isLoading && (
      ...
      {!isLoading && grouped.length === 0 && (
        <div className="text-center py-12 text-slate-500">
          <p className="text-base sm:text-lg">No upcoming events</p>
          <p className="text-xs sm:text-sm mt-1">Events from your connected calendars will appear here</p>
        </div>
      )}
      ...
      {grouped.map(([dateKey, dayEvents]) => {
        ...
        return (
          <div key={dateKey} className="mb-4 sm:mb-6">
            {/* Date header */}
            <div className="flex items-center gap-2 sm:gap-3 mb-2 sticky top-0 bg-slate-950/90 backdrop-blur-sm py-1 z-10">
              <div className={`text-center w-10 sm:w-12 ${isToday(date) ? 'text-blue-400' : 'text-slate-400'}`}>
                <div className="text-[10px] sm:text-xs uppercase">{format(date, 'EEE')}</div>
                <div className={`text-lg sm:text-xl font-bold ${
                  isToday(date) ? 'bg-blue-600 text-white w-7 h-7 sm:w-8 sm:h-8 rounded-full flex items-center justify-center mx-auto' : ''
                }`}>
                  {format(date, 'd')}
                </div>
              </div>
            ...
            {/* Events list */}
            <div className="flex flex-col gap-1.5 ml-12 sm:ml-14">
              {dayEvents.map(ev => (
...
function AgendaEventRow({ event, onClick }: { event: CalendarEvent; onClick: () => void }) {
  ...
  return (
    <button
      onClick={onClick}
      className="flex items-start gap-2 sm:gap-3 p-2 sm:p-3 rounded-lg sm:rounded-xl bg-slate-900/60 hover:bg-slate-800/60 transition-colors text-left w-full min-h-[48px] sm:min-h-[56px]"
    >
